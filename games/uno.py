  ...
  def output(self, objs : Any | List[str | FormattedText], sep='', end='\n'):
   ...
   for obj in objs:
    if isinstance(obj, FormattedText):
      colored_print(obj._text, end='', fg_color=obj._fg_color, bg_color=obj._bg_color,format=obj._format)     
    elif isinstance(obj, str):
      print(obj, end='')
    elif isinstance(obj, Color):
      self.output(FormattedText(text=f'{self._symbol}',
     ...
     for card in obj.cards(): self.output(card)
    elif isinstance(obj, Card):
      self.output(FormattedText(text=f"{obj._color._symbol} {obj._rank}",
...
class WebUI(UI):
  ...
  def __init__(self) -> None: 
    ...
    pywebio.output.put_scrollable(
      pywebio.output.put_scope(self._main_scope),
        height=550, keep_bottom=True)
  ...
  def output(self, objs, end=None, sep=''):
   with pywebio.output.use_scope(self._main_scope):
    ...
    for obj in objs:   
      ...
      if isinstance(obj, FormattedText):
        pywebio.output.style(
          [
            pywebio.output.put_text(obj._text, inline=True,
        ...
        if obj == '\n':
          pywebio.output.put_text('',  inline=False,scope=self._main_scope)
        else:
          pywebio.output.put_text(obj, inline=True,scope=self._main_scope)
      elif isinstance(obj, Color):
        pywebio.output.put_text(f'{obj._symbol}', inline=True).style(f'color: {obj._html_name}')
      elif isinstance(obj, Hand):
        ...
        pywebio.output.put_text(f'{obj._color.symbol()} {obj._rank}',
                                inline=True,scope=self._main_scope).style(
          ";".join([
    ...
    if end: self.output(end)

    pywebio.output.scroll_to(position=pywebio.output.Position.BOTTOM, scope=self._main_scope)
...
class Game:
  ...
  def play_round(self, discard, hands, turn, current_color):
    ...
    while True:
      ...
      if choice is not None and 1<=choice<=len(current_hand):
        if self.check_card(current_hand[choice-1], current_card, current_color):
          break
  ...
  def play_uno(self):
    ...
    while True:
      self._ui.clear()
      self._ui.output([f'Player {turn+1}, it\'s your turn.\n',
      ...
      if result==0:
        ...
        if played_card.color().symbol()=='W':
          self._ui.output([f'Which color would you like to change the pile to? Type the letter [']
                          + Deck.COLORS + [']\n'], sep='')
          while True:
...
class Color:
  ...
  def __str__(self):
    return colored_sprint(FormattedText(text=f"{self._symbol}",
                          fg_color=self._number,
                          bg_color=None,
                          format=None))
...
class Deck:
  """Deck of Uno cards."""
  COLORS = [Color('R', "red", 1), Color('G', "green", 28), Color('B', "blue", 20), Color('Y', "orange", 3)]
  ...
  def __init__(self):
    ...
    self._cards.add(
      ...
      [Card(Color('W', 'black', 0), 'wild'),
       Card(Color('W', 'black', 0), 'wild draw 4')] * 4)
