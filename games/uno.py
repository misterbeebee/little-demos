...
import os
from pywebio.input import input, FLOAT, TEXT, NUMBER
...
def play_round(discard, hands, turn, current_color):
  ...
  if current_color is None:
    put_text(f'Last played card, to match: {current_card}\n')
  else:
    put_text(f'Last played card, to match: {current_card} ({current_color})\n')
  current_hand=hands[turn].cardlist()
  for i in range(len(current_hand)):
    put_text(f'{i+1}: {current_hand[i]}')
    ...
    if True:
      ...
      if 1<=choice<=len(current_hand):
       if check_card(current_hand[choice-1], current_card, current_color):
         break
    put_text('Bad option. Please choose another card.')
...
def play_uno():
  ...
  while True:
    put_text("How many players? [1 to 10]")
    num_players = input(type=NUMBER)
    #if not num_players.isdigit():
      #print("Bad number. Please try again.")
      #continue
    num_players = int(num_players)
    if not 1 <= num_players <= 10:
      put_text("1 to 10 players only.")
      continue
    break
  ...
  while True:
    clear()
    put_text(f'Player {turn+1}, it\'s your turn.\nMake sure other players are looking away and press enter.')
    input(type=TEXT)

    for i in range(num_players):
      if len(hands[i])==1:
        put_text(f'Player {i+1} has 1 card.')
      else:         
        put_text(f'Player {i+1} has {len(hands[i])} cards.')
    ...
    if result==0:
      ...
      if played_card.color().symbol()=='W':
        put_text('Which color would you like to change the pile to? Type the letter.')
        while True:
          current_color=input(type=TEXT).upper()
          if current_color in COLOR_SYMBOLS:
            break
          put_text('Bad option. Please choose another color.')
